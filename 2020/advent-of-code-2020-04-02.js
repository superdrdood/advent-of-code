...
ecl:amb`;
  ...
  var array20200402 = array20200402.split("\n\n");

  function calculate() {

    var valid = 0;

    for (i = 0; i < array20200402.length; i++ ) {

      array20200402[i] = array20200402[i].replace(/\n/g, " ");

      let arraytest = array20200402[i].split(" ");
      
      if (arraytest.length == 8) {
        let validmaybe = 0;

        for (j = 0; j < arraytest.length; j++) {
          if (isValidCode(arraytest[j])) {
            validmaybe++
          }
        }
      ...
      } else if ( arraytest.length == 7) {

          let cidcheck = 0;
          let validmaybe = 0;
          
          for (j = 0; j < arraytest.length; j++) {
            if (arraytest[j].split(":")[0] != "cid") {
              cidcheck++;
              if (isValidCode(arraytest[j])) {
                validmaybe++
              }
  ...
  function isValidCode(valuepair) {
    ...
    switch(key) {
      ...
      case "hgt":
        if (value.length == 5) {
          if (value.substring(0, 3) >= 150 && value.substring(0, 3) <= 193 && value.substring(3, 5) == "cm") {
            return true;
          } else {
            return false;
          }
        } else if (value.length == 4) {
          if (value.substring(0, 2) >= 59 && value.substring(0, 2) <= 76 && value.substring(2, 4) == "in") {
            return true;
          } else {
            return false;
          }
        ...
        break;
      case "hcl":
        let regexHCL = RegExp("^[0-9a-f]+$");
        if (value.length == 7 && regexHCL.test(value.substring(1, 7)) && value.substring(0, 1) == "#" ) {
          return true;
        } else {
          return false;
        }
      ...
      case "ecl":
        ...
        break;
      case "pid":
        let regexPID = RegExp("^[0-9]+$");
        if (value.length == 9 && regexPID.test(value)) {
          return true;
        } else {
          return false;
        }
