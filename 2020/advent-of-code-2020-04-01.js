...
var array20200401 = array20200401.split("\n\n");
...
var aoc20200401 = {
  ...
  calculate : function() {
    ...
    for (i = 0; i < array20200401.length; i++ ) {

      array20200401[i] = array20200401[i].replace(/\n/g, " ");

      var arraytest20200401 = array20200401[i].split(" ");

      if (arraytest20200401.length == 8) {
        valid20200401++
      } else if ( arraytest20200401.length == 7) {
        var validmaybe20200401 = 0;
        for (j = 0; j < arraytest20200401.length; j++) {
          if (arraytest20200401[j].split(":")[0] != "cid") {
            validmaybe20200401++;
          }
        }
        if (validmaybe20200401 == 7) {
          valid20200401++;
        }
      }
    }
    return valid20200401;
  },

  answer : function() {
    answerMe("2020-04-01",aoc20200401.calculate());
  }
}
